...
4. Commit all changes, create an annotated tag (e.g. `v1.0.13`), and push branch and tag. The release workflow creates the GitHub Release from the changelog.
...
```bash
git add src/EventSubscriber src/ConsoleDebugHolder.php src/ConsoleDebugRegistry.php src/Resources/config/services.yaml tests/Unit/EventSubscriber README.md docs/
git status   # review
git commit -m "Release v1.0.13: FrankenPHP worker-safe registry without kernel reset"
git tag -a v1.0.13 -m "Release v1.0.13: FrankenPHP worker-safe registry without kernel reset"
