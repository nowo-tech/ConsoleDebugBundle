...
2. **Product behavior** — what **ConsoleDebugBundle** guarantees to applications that integrate it (see [`USAGE.md`](USAGE.md), [`CONFIGURATION.md`](CONFIGURATION.md), [`INSTALLATION.md`](INSTALLATION.md)). **PHPUnit** and **PHPStan** (and **Vitest** when applicable) enforce contracts in CI where applicable.
...
**Goal:** Browser console debugging for Symfony via **`cdbg()`**, with role-gated collection and HTML script injection for authorized users.
